function changeQuantity(productId, change, price) {
    ...
    const sumPrice = document.querySelector(".total-text")
    ...
    let currentTotal = parseInt(sumPrice.innerText.replace('đ', ''));
    ...
    // Cập nhật lại tổng tiền và thêm ký tự "đ"
    sumPrice.innerHTML = currentTotal + '<span>đ</span>';
...
function addToCart(productId, price) {
    ...
    const uri = "/gio-hang/them-moi-chi-tiet-giohang";
    $.ajax({
        url: uri,
        type: "POST",
        data: {
        ...
        success: function (response) {
            ...
            // Kiểm tra element cart-total tồn tại
            const cartTotal = $("#cart-total");
            if (cartTotal.length) {
                cartTotal.text("Tổng tiền: " + response.total);
            }

            cartNumber.innerHTML = parseInt(cartNumber.textContent) + 1;
            console.log(cartNumber)
...
function removeCart(idChitietgiohang) {
    const cartNumber = document.querySelector("#cart-number")
    let checkRemove = confirm("Bạn có muốn xóa sản phẩm này?")
    if (checkRemove) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let curCartNumber = Number(cartNumber.innerText) - 1;
                cartNumber.innerHTML = curCartNumber
